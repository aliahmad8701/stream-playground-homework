...
public class Homework1 {
    ...
    public Optional<String> streamPipeline2() {
        countries.stream().map(Country::getName).filter(s -> s.toLowerCase().contains("island")).findFirst().get();
        return null;
    }
    ...
    public void streamPipeline3() {
        countries.stream().map(Country::getName).filter(s -> { String t = s.toLowerCase(); 
	return t.charAt(0) == t.charAt(t.length() - 1); }).forEach(System.out::println);
    }
    ...
    public void streamPipeline8() {
        countries.stream().sorted(Comparator.comparingInt(country -> country.getTimezones().size())).forEach(country -> System.out.printf("@code %s:%d\n", country.getName(), country.getTimezones().size()));
    }
    ...
    public double streamPipeline12() {
       countries.stream().map(Country::getName).mapToInt(String::length).average().getAsDouble();
        return 0;
    }
    ...
    public void streamPipeline15() {
       countries.stream().filter(country -> country.getArea() != null).filter(country -> country.getArea().compareTo(BigDecimal.ONE) < 0).map(Country::getName).forEach(System.out::println);
    }
